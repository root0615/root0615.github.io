...
또 하나 궁금한 점은 데이터 배열(32, 784)와 가중치 배열(256, 784)를 행렬곱해야 하는데 안되잖아요?<br>
그래서 가중치 배열을 전치(열과 행 뒤집음) 해줍니다 (W -> Wᵀ)<br>
그러면 행렬곱이 가능해져서 위의 수식과 같습니다.
...
|output_padding|출력 한쪽에 추가하는 픽셀.<br>stride=2일 때 홀수 크기 입력으로 발생하는 크기 불일치를 조정할 때 사용.|
